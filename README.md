...
narrows as threads are added — to 1.7× on the synthetic corpus and 2.3× on real
...
- **Vinyl scales better with threads than libFLAC.** From 1 to 8 threads it
  gains 3.5× (synthetic) and 5.1× (real audio) on encode, against libFLAC's
  2.3× and 4.2×. Both codecs take a thread count — `vinyl -j N` and
  `flac -j N` — so the comparison can be made at parity.
- **Decoding gets faster with more threads, and that is where Vinyl wins on
  wall clock**: 218 MB/s against libFLAC's 192 MB/s on real audio at eight
...
- **The gap is a constant factor, not a content effect.** Every curve is
  ...
  solo instrument, clean and noisy speech — so the ×2.6 encode gap at eight
  threads and the ×3.2 gap at one hold on the median unit and at essentially
...
- **Decode at eight threads is the one place Vinyl is ahead on wall clock.**
  ...
  median unit, 218 MB/s against 192 MB/s on corpus totals — and libFLAC has
  nothing to answer with: its decoder takes no `-j`, which is why it appears
  once rather than twice.
- **Per core it is ~3.2× behind on encode and ~3.9× on decode** — the
